...
void the_display_recup_elf_descriptor_64(file_t *file,
                            version_64_t *version, int tmp)
{
    if (version->shdr[tmp].sh_type == SHT_SYMTAB) {
        ...
        for (long unsigned int i = 0; i < (version->shdr[tmp].sh_size /
            ...
            file->c = gettype_64(&version->sym[i], version->shdr);
            display_result_64(version, file->c, &version->sym[i]);
        }
    }
}
...
int obtain_the_elf_descriptor_64(file_t *file)
{
    ...
    for (int tmp = 0; tmp < version.ehdr->e_shnum; tmp++) {
       the_display_recup_elf_descriptor_64(file, &version, tmp);
    }
    return 0;
}